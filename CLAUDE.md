...
- **Background jobs**: Quartz scheduler is wired up in `Program.cs`; most one-off migration jobs are commented out. `PublishMagazinesJob` (1st of the month) and `IssueRemindersJob` (daily) have schedules written but commented out, pending approval. Jobs live in `/BackgroundJobs/`.
