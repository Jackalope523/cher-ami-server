...
namespace CherAmiAPI.Endpoints.Issues
{
    public class PostCountEndpoint(ApplicationDbContext ctx) : EndpointWithoutRequest
    {
        ...
        public override async Task HandleAsync(CancellationToken cancellationToken)
        {
            ...
            long circleId = await ctx.Users.Where(u => u.Id == userId).Select(u => u.CircleId).SingleAsync(cancellationToken) ?? throw new NotFoundException("User does not belong to a circle.");
